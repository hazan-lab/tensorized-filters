...
from tensorized_filters.training.dataloader import DistributedDataloader
from tensorized_filters.training.distributed import (
    cleanup_distributed,
    find_checkpoint,
    load_checkpoint,
    save_checkpoint,
...
from tensorized_filters.models.tensorized_mixed.config import TensorizedSTUMixedConfig
from tensorized_filters.models.tensorized_mixed.model import TensorizedSTUMixed
from tensorized_filters.utils.filters import get_tensorized_spectral_filters
from tensorized_filters.training.gc import GarbageCollection
from tensorized_filters.training.schedulers import linear_decay_with_warmup
...
def main():
    ...
    config = TensorizedSTUMixedConfig(
        bsz=bsz,
        n_embd=n_embd,
        n_heads=n_heads,
        n_layers=n_layers,
    ...
    phi = get_tensorized_spectral_filters(seq_len, num_eigh, use_hankel_L=True, device=device, dtype=torch_dtype)
    model = TensorizedSTUMixed(config, phi)
    if torch_compile:
        model = torch.compile(model)
        if main_process:
            logger.info(f"PyTorch Compiler Enabled?: {torch_compile}")
    ...
    torch.serialization.add_safe_globals([TensorizedSTUMixedConfig])
    ...
    dataset = "/scratch/gpfs/mn4560/flash-stu/training/data/fineweb-edu-10B"
    total_tokens = 525_000_000
    num_steps = total_tokens // global_bsz  # Number of steps for one epoch
    max_steps = num_steps * num_epochs

    if main_process:
        logger.info(f"Total (desired) batch size: {global_bsz}")
        logger.info(f"=> Calculated gradient accumulation steps: {gradient_accumulation_steps}")
        logger.info(f"Training on {max_steps} steps")
    ...
    for step in range(start_step + 1, max_steps + 1):
        ...
        if step == 1 or step % (eval_period // dilation) == 0 or last_step:
            ...
            val_loader.reset()
            with torch.no_grad():
                for _, batch in zip(range(val_steps), val_loader, strict=False):
            ...
            if step > 0 and (step % save_period == 0 or last_step):
                if val_loss.item() < best_val_loss:
                    ...
                    dist.barrier()
                    model_checkpoint, optim_checkpoint = get_state_dict(model, optimizer, options=state_dict_options)
                    if main_process:
        ...
        dt = t1 - t0
        toks_processed = train_loader.bsz * train_loader.seq_len * gradient_accumulation_steps * world_size
