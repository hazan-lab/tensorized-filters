...
from tensorized_filters.models.tensorized_stu_no_attn.config import TensorizedSTUNoAttnConfig
from tensorized_filters.models.tensorized_stu_no_attn.model import TensorizedSTUNoAttn
...
def main():
    ...
    config = TensorizedSTUNoAttnConfig(
        bsz=bsz,
        n_embd=n_embd,
        n_heads=n_heads,
        n_layers=n_layers,
    ...
    phi = get_tensorized_spectral_filters(seq_len, num_eigh, use_hankel_L, device, torch_dtype)
    model = TensorizedSTUNoAttn(config, phi)
    if torch_compile:
    ...
    torch.serialization.add_safe_globals([TensorizedSTUNoAttnConfig])
