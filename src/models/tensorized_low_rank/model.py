...
from tensorized_filters.utils.modules.tensorized_stu_low_rank import STU
...
from tensorized_filters.layers.tensorized_stu_low_rank import STULayer
...
class TensorizedLowRank(PreTrainedModel):
    config_class = TensorizedLowRankConfig

    def __init__(self, config, phi) -> None:
        super(TensorizedLowRank, self).__init__(config)
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            for name, param in module.named_parameters(recurse=False):
                if name == "M_inputs" or name == "M_filters":
                    torch.nn.init.xavier_normal_(param)
                elif name == "M_phi_plus":
                    torch.nn.init.xavier_normal_(param)
                elif name == "M_phi_minus" and not module.use_hankel_L:
                    torch.nn.init.xavier_normal_(param)
                elif name == "M_i" or name == "M_j":
                    torch.nn.init.xavier_normal_(param)
                elif name == "M_i_plus" or name == "M_j_plus":
                    torch.nn.init.xavier_normal_(param)
                elif name == "M_i_minus" or name == "M_j_minus":
                    torch.nn.init.xavier_normal_(param)
